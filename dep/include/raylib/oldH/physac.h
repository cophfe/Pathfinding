...
*       Forces library to define struct RLVector2 data type (float x; float y)
*
...
// RLMatrix2x2 type (used for polygon shape rotation matrix)
typedef struct RLMatrix2x2
{
	float m00;
	float m01;
	float m10;
...
typedef struct PhysicsVertexData
{
	...
	RLVector2 positions[PHYSAC_MAX_VERTICES];     // Vertex positions vectors
	RLVector2 normals[PHYSAC_MAX_VERTICES];       // Vertex normals vectors
} PhysicsVertexData;
...
typedef struct PhysicsShape
{
	...
	RLMatrix2x2 transform;                        // Vertices transform matrix 2x2
} PhysicsShape;
...
typedef struct PhysicsBodyData
{
	...
	RLVector2 position;                           // Physics body shape pivot
	RLVector2 velocity;                           // Current linear velocity applied to position
	RLVector2 force;                              // Current linear force (reset to 0 every step)
...
typedef struct PhysicsManifoldData
{
	...
	RLVector2 normal;                             // Normal direction vector from 'a' to 'b'
	RLVector2 contacts[2];                        // Points of contact during collision
...
// Physics system management
	...
	PHYSACDEF PhysicsBody CreatePhysicsBodyCircle(RLVector2 pos, float radius, float density);                    // Creates a new circle physics body with generic parameters
	PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(RLVector2 pos, float width, float height, float density);    // Creates a new rectangle physics body with generic parameters
	PHYSACDEF PhysicsBody CreatePhysicsBodyPolygon(RLVector2 pos, float radius, int sides, float density);        // Creates a new polygon physics body with generic parameters
	PHYSACDEF void DestroyPhysicsBody(PhysicsBody body);                                                        // Destroy a physics body

	// Physic body forces
	PHYSACDEF void PhysicsAddForce(PhysicsBody body, RLVector2 force);                                            // Adds a force to a physics body
	PHYSACDEF void PhysicsAddTorque(PhysicsBody body, float amount);                                            // Adds an angular force to a physics body
	PHYSACDEF void PhysicsShatter(PhysicsBody body, RLVector2 position, float force);                             // Shatters a polygon shape physics body to little physics bodies with explosion force
	...
	PHYSACDEF RLVector2 GetPhysicsShapeVertex(PhysicsBody body, int vertex);                                      // Returns transformed position of a body shape (body position + vertex transformed position)
...
#define PHYSAC_VECTOR_ZERO      CLITERAL(RLVector2){ 0.0f, 0.0f }
...
static RLVector2 gravityForce = { 0.0f, 9.81f };              // Physics world gravity force
...
static PhysicsVertexData CreateRectanglePolygon(RLVector2 pos, RLVector2 size);                                 // Creates a rectangle polygon shape based on a min and max positions
...
static void FindIncidentFace(RLVector2* v0, RLVector2* v1, PhysicsShape ref, PhysicsShape inc, int index);      // Finds two polygon shapes incident face
...
static RLVector2 MathVector2Product(RLVector2 vector, float value);                                             // Returns the product of a vector and a value
static float MathVector2CrossProduct(RLVector2 v1, RLVector2 v2);                                               // Returns the cross product of two vectors
static float MathVector2SqrLen(RLVector2 vector);                                                             // Returns the len square root of a vector
static float MathVector2DotProduct(RLVector2 v1, RLVector2 v2);                                                 // Returns the dot product of two vectors
static inline float MathVector2SqrDistance(RLVector2 v1, RLVector2 v2);                                         // Returns the square root of distance between two vectors
static void MathVector2Normalize(RLVector2* vector);                                                          // Returns the normalized values of a vector
static RLVector2 MathVector2Add(RLVector2 v1, RLVector2 v2);                                                      // Returns the sum of two given vectors
static RLVector2 MathVector2Subtract(RLVector2 v1, RLVector2 v2);                                                 // Returns the subtract of two given vectors
static RLMatrix2x2 MathMatFromRadians(float radians);                                                         // Returns a matrix 2x2 from a given radians value
static inline RLMatrix2x2 MathMatTranspose(RLMatrix2x2 matrix);                                                 // Returns the transpose of a given matrix 2x2
static inline RLVector2 MathMatVector2Product(RLMatrix2x2 matrix, RLVector2 vector);                              // Returns product between matrix 2x2 and vector
static int MathVector2Clip(RLVector2 normal, RLVector2* faceA, RLVector2* faceB, float clip);                     // Returns clipping value based on a normal and two faces
static RLVector2 MathTriangleBarycenter(RLVector2 v1, RLVector2 v2, RLVector2 v3);                                  // Returns the barycenter of a triangle given by 3 points
...
PHYSACDEF PhysicsBody CreatePhysicsBodyCircle(RLVector2 pos, float radius, float density)
{
...
PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(RLVector2 pos, float width, float height, float density)
{
	...
	if (id != -1)
	{
		...
		body->shape.vertexData = CreateRectanglePolygon(pos, CLITERAL(RLVector2){ width, height });

		// Calculate centroid and moment of inertia
		RLVector2 center = { 0.0f, 0.0f };
		float area = 0.0f;
		float inertia = 0.0f;
		...
		for (unsigned int i = 0; i < body->shape.vertexData.vertexCount; i++)
		{
			...
			RLVector2 p1 = body->shape.vertexData.positions[i];
			unsigned int nextIndex = (((i + 1) < body->shape.vertexData.vertexCount) ? (i + 1) : 0);
			RLVector2 p2 = body->shape.vertexData.positions[nextIndex];
...
PHYSACDEF PhysicsBody CreatePhysicsBodyPolygon(RLVector2 pos, float radius, int sides, float density)
{
	...
	if (id != -1)
	{
		...
		{
			// Triangle vertices, third vertex implied as (0, 0)
			RLVector2 position1 = body->shape.vertexData.positions[i];
			unsigned int nextIndex = (((i + 1) < body->shape.vertexData.vertexCount) ? (i + 1) : 0);
			RLVector2 position2 = body->shape.vertexData.positions[nextIndex];
...
PHYSACDEF void PhysicsAddForce(PhysicsBody body, RLVector2 force)
{
...
PHYSACDEF void PhysicsShatter(PhysicsBody body, RLVector2 position, float force)
{
	if (body != NULL)
	{
		if (body->shape.type == PHYSICS_POLYGON)
		{
			...
			{
				RLVector2 positionA = body->position;
				RLVector2 positionB = MathMatVector2Product(body->shape.transform, MathVector2Add(body->position, vertexData.positions[i]));
				unsigned int nextIndex = (((i + 1) < vertexData.vertexCount) ? (i + 1) : 0);
				RLVector2 positionC = MathMatVector2Product(body->shape.transform, MathVector2Add(body->position, vertexData.positions[nextIndex]));
			...
			if (collision)
			{
				...
				RLVector2 bodyPos = body->position;
				RLVector2* vertices = (RLVector2*)PHYSAC_MALLOC(sizeof(RLVector2) * count);
				RLMatrix2x2 trans = body->shape.transform;
				...
				for (int i = 0; i < count; i++)
				{
					...
					RLVector2 center = MathTriangleBarycenter(vertices[i], vertices[nextIndex], PHYSAC_VECTOR_ZERO);
					center = MathVector2Add(bodyPos, center);
					RLVector2 offset = MathVector2Subtract(center, bodyPos);
					...
					for (unsigned int j = 0; j < vertexData.vertexCount; j++)
					{
						...
						RLVector2 face = MathVector2Subtract(vertexData.positions[nextVertex], vertexData.positions[j]);

						vertexData.normals[j] = CLITERAL(RLVector2) { face.y, -face.x };
					...
					{
						// Triangle vertices, third vertex implied as (0, 0)
						RLVector2 p1 = body->shape.vertexData.positions[j];
						unsigned int nextVertex = (((j + 1) < body->shape.vertexData.vertexCount) ? (j + 1) : 0);
						RLVector2 p2 = body->shape.vertexData.positions[nextVertex];
					...
					RLVector2 pointA = body->position;
					RLVector2 pointB = MathVector2Subtract(vertexData.positions[1], vertexData.positions[0]);
					pointB.x /= 2.0f;
					pointB.y /= 2.0f;
					RLVector2 forceDirection = MathVector2Subtract(MathVector2Add(pointA, MathVector2Add(vertexData.positions[0], pointB)), body->position);
...
PHYSACDEF RLVector2 GetPhysicsShapeVertex(PhysicsBody body, int vertex)
{
...
static PhysicsVertexData CreateDefaultPolygon(float radius, int sides)
{
	...
	for (int i = 0; i < (int)data.vertexCount; i++)
	{
		...
		RLVector2 face = MathVector2Subtract(data.positions[nextIndex], data.positions[i]);

		data.normals[i] = CLITERAL(RLVector2) { face.y, -face.x };
...
static PhysicsVertexData CreateRectanglePolygon(RLVector2 pos, RLVector2 size)
{
	PhysicsVertexData data = { 0 };
	data.vertexCount = 4;

	// Calculate polygon vertices positions
	data.positions[0] = CLITERAL(RLVector2) { pos.x + size.x / 2, pos.y - size.y / 2 };
	data.positions[1] = CLITERAL(RLVector2) { pos.x + size.x / 2, pos.y + size.y / 2 };
	data.positions[2] = CLITERAL(RLVector2) { pos.x - size.x / 2, pos.y + size.y / 2 };
	data.positions[3] = CLITERAL(RLVector2) { pos.x - size.x / 2, pos.y - size.y / 2 };
...
static void SolveCircleToCircle(PhysicsManifold manifold)
{
	...
	RLVector2 normal = MathVector2Subtract(bodyB->position, bodyA->position);
	...
	if (distance == 0.0f)
	{
		...
		manifold->normal = CLITERAL(RLVector2) { 1.0f, 0.0f };
		manifold->contacts[0] = bodyA->position;
	}
	else
	{
		manifold->penetration = radius - distance;
		manifold->normal = CLITERAL(RLVector2) { normal.x / distance, normal.y / distance }; // Faster than using MathVector2Normalize() due to sqrt is already performed
		manifold->contacts[0] = CLITERAL(RLVector2) { manifold->normal.x* bodyA->shape.radius + bodyA->position.x, manifold->normal.y* bodyA->shape.radius + bodyA->position.y };
	}
...
static void SolveCircleToPolygon(PhysicsManifold manifold)
{
	...
	RLVector2 center = bodyA->position;
	...
	RLVector2 v1 = vertexData.positions[faceNormal];
	int nextIndex = (((faceNormal + 1) < (int)vertexData.vertexCount) ? (faceNormal + 1) : 0);
	RLVector2 v2 = vertexData.positions[nextIndex];

	// Check to see if center is within polygon
	if (separation < PHYSAC_EPSILON)
	{
		manifold->contactsCount = 1;
		RLVector2 normal = MathMatVector2Product(bodyB->shape.transform, vertexData.normals[faceNormal]);
		manifold->normal = CLITERAL(RLVector2) { -normal.x, -normal.y };
		manifold->contacts[0] = CLITERAL(RLVector2) { manifold->normal.x* bodyA->shape.radius + bodyA->position.x, manifold->normal.y* bodyA->shape.radius + bodyA->position.y };
		manifold->penetration = bodyA->shape.radius;
		return;
	}
	...
	if (dot1 <= 0.0f) // Closest to v1
	{
		...
		manifold->contactsCount = 1;
		RLVector2 normal = MathVector2Subtract(v1, center);
	...
	else if (dot2 <= 0.0f) // Closest to v2
	{
		...
		manifold->contactsCount = 1;
		RLVector2 normal = MathVector2Subtract(v2, center);
	...
	{
		RLVector2 normal = vertexData.normals[faceNormal];
...
static void SolvePolygonToPolygon(PhysicsManifold manifold)
{
	...
	RLVector2 v1 = refData.positions[referenceIndex];
	referenceIndex = (((referenceIndex + 1) < (int)refData.vertexCount) ? (referenceIndex + 1) : 0);
	RLVector2 v2 = refData.positions[referenceIndex];
	...
	RLVector2 sidePlaneNormal = MathVector2Subtract(v2, v1);
	MathVector2Normalize(&sidePlaneNormal);

	// Orthogonalize
	RLVector2 refFaceNormal = { sidePlaneNormal.y, -sidePlaneNormal.x };
	...
	if (MathVector2Clip(CLITERAL(RLVector2) { -sidePlaneNormal.x, -sidePlaneNormal.y }, & incidentFace[0], & incidentFace[1], negSide) < 2) return;
	if (MathVector2Clip(sidePlaneNormal, &incidentFace[0], &incidentFace[1], posSide) < 2) return;

	// Flip normal if required
	manifold->normal = (flip ? CLITERAL(RLVector2) { -refFaceNormal.x, -refFaceNormal.y } : refFaceNormal);
...
static void InitializePhysicsManifolds(PhysicsManifold manifold)
{
	...
	for (unsigned int i = 0; i < manifold->contactsCount; i++)
	{
		...
		RLVector2 radiusA = MathVector2Subtract(manifold->contacts[i], bodyA->position);
		RLVector2 radiusB = MathVector2Subtract(manifold->contacts[i], bodyB->position);

		RLVector2 crossA = MathVector2Product(radiusA, bodyA->angularVelocity);
		RLVector2 crossB = MathVector2Product(radiusB, bodyB->angularVelocity);

		RLVector2 radiusV = { 0.0f, 0.0f };
		...
		if (MathVector2SqrLen(radiusV) < (MathVector2SqrLen(CLITERAL(RLVector2) { (float)(gravityForce.x * deltaTime / 1000), (float)(gravityForce.y * deltaTime / 1000) }) + PHYSAC_EPSILON)) manifold->restitution = 0;
	}
}
...
static void IntegratePhysicsImpulses(PhysicsManifold manifold)
{
	...
	for (unsigned int i = 0; i < manifold->contactsCount; i++)
	{
		...
		RLVector2 impulseV = { manifold->normal.x * impulse, manifold->normal.y * impulse };

		if (bodyA->enabled)
		{
			bodyA->velocity.x += bodyA->inverseMass * (-impulseV.x);
			bodyA->velocity.y += bodyA->inverseMass * (-impulseV.y);
			if (!bodyA->freezeOrient) bodyA->angularVelocity += bodyA->inverseInertia * MathVector2CrossProduct(radiusA, CLITERAL(RLVector2){ -impulseV.x, -impulseV.y });
		}
		...
		RLVector2 tangent = { radiusV.x - (manifold->normal.x * MathVector2DotProduct(radiusV, manifold->normal)), radiusV.y - (manifold->normal.y * MathVector2DotProduct(radiusV, manifold->normal)) };
		...
		// Apply coulumb's law
		RLVector2 tangentImpulse = { 0.0f, 0.0f };
		if (absImpulseTangent < impulse * manifold->staticFriction) tangentImpulse = CLITERAL(RLVector2) { tangent.x* impulseTangent, tangent.y* impulseTangent };
		else tangentImpulse = CLITERAL(RLVector2) { tangent.x * -impulse * manifold->dynamicFriction, tangent.y * -impulse * manifold->dynamicFriction };
		...
		if (bodyA->enabled)
		{
			...
			if (!bodyA->freezeOrient) bodyA->angularVelocity += bodyA->inverseInertia * MathVector2CrossProduct(radiusA, CLITERAL(RLVector2){ -tangentImpulse.x, -tangentImpulse.y });
		}
...
static void CorrectPhysicsPositions(PhysicsManifold manifold)
{
	...
	RLVector2 correction = { 0.0f, 0.0f };
...
static RLVector2 GetSupport(PhysicsShape shape, RLVector2 dir)
{
	float bestProjection = -PHYSAC_FLT_MAX;
	RLVector2 bestVertex = { 0.0f, 0.0f };
	PhysicsVertexData data = shape.vertexData;

	for (unsigned int i = 0; i < data.vertexCount; i++)
	{
		RLVector2 vertex = data.positions[i];
...
static float FindAxisLeastPenetration(int* faceIndex, PhysicsShape shapeA, PhysicsShape shapeB)
{
	...
	for (unsigned int i = 0; i < dataA.vertexCount; i++)
	{
		...
		RLVector2 normal = dataA.normals[i];
		RLVector2 transNormal = MathMatVector2Product(shapeA.transform, normal);

		// Transform face normal into B shape's model space
		RLMatrix2x2 buT = MathMatTranspose(shapeB.transform);
		normal = MathMatVector2Product(buT, transNormal);

		// Retrieve support point from B shape along -n
		RLVector2 support = GetSupport(shapeB, CLITERAL(RLVector2){ -normal.x, -normal.y });

		// Retrieve vertex on face from A shape, transform into B shape's model space
		RLVector2 vertex = dataA.positions[i];
...
static void FindIncidentFace(RLVector2* v0, RLVector2* v1, PhysicsShape ref, PhysicsShape inc, int index)
{
	PhysicsVertexData refData = ref.vertexData;
	PhysicsVertexData incData = inc.vertexData;

	RLVector2 referenceNormal = refData.normals[index];
...
static int MathVector2Clip(RLVector2 normal, RLVector2* faceA, RLVector2* faceB, float clip)
{
	int sp = 0;
	RLVector2 out[2] = { *faceA, *faceB };
	...
	if ((distanceA * distanceB) < 0.0f)
	{
		...
		out[sp] = *faceA;
		RLVector2 delta = MathVector2Subtract(*faceB, *faceA);
		delta.x *= alpha;
		delta.y *= alpha;
...
static RLVector2 MathTriangleBarycenter(RLVector2 v1, RLVector2 v2, RLVector2 v3)
{
...
{
#if defined(_WIN32)
	Module_QueryPerformanceFrequency((unsigned long long int*) & frequency);
#endif
...
#if defined(_WIN32)
	Module_QueryPerformanceCounter((unsigned long long int*) & value);
#endif
...
static inline RLVector2 MathVector2Product(RLVector2 vector, float value)
{
	RLVector2 result = { -value * vector.y, value * vector.x };
	return result;
}

// Returns the cross product of two vectors
static inline float MathVector2CrossProduct(RLVector2 v1, RLVector2 v2)
{
	return (v1.x * v2.y - v1.y * v2.x);
}

// Returns the len square root of a vector
static inline float MathVector2SqrLen(RLVector2 vector)
{
	return (vector.x * vector.x + vector.y * vector.y);
}

// Returns the dot product of two vectors
static inline float MathVector2DotProduct(RLVector2 v1, RLVector2 v2)
{
	return (v1.x * v2.x + v1.y * v2.y);
}

// Returns the square root of distance between two vectors
static inline float MathVector2SqrDistance(RLVector2 v1, RLVector2 v2)
{
	RLVector2 dir = MathVector2Subtract(v1, v2);
	return MathVector2DotProduct(dir, dir);
}

// Returns the normalized values of a vector
static void MathVector2Normalize(RLVector2* vector)
{
	float length, ilength;

	RLVector2 aux = *vector;
...
static inline RLVector2 MathVector2Add(RLVector2 v1, RLVector2 v2)
{
...
static inline RLVector2 MathVector2Subtract(RLVector2 v1, RLVector2 v2)
{
	RLVector2 result = { v1.x - v2.x, v1.y - v2.y };
	return result;
}
...
static RLMatrix2x2 MathMatFromRadians(float radians)
{
	float cos = cosf(radians);
	float sin = sinf(radians);

	RLMatrix2x2 result = { cos, -sin, sin, cos };
	return result;
}

// Returns the transpose of a given matrix 2x2
static inline RLMatrix2x2 MathMatTranspose(RLMatrix2x2 matrix)
{
	RLMatrix2x2 result = { matrix.m00, matrix.m10, matrix.m01, matrix.m11 };
	return result;
}

// Multiplies a vector by a matrix 2x2
static inline RLVector2 MathMatVector2Product(RLMatrix2x2 matrix, RLVector2 vector)
{
	RLVector2 result = { matrix.m00 * vector.x + matrix.m01 * vector.y, matrix.m10 * vector.x + matrix.m11 * vector.y };
	return result;
}
